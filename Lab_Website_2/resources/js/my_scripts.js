...
function loadStatsPage(){
	...
	for (var rowNum = 2; rowNum < table.row.length ;rowNum++){
		var home = table.row[rowNum].cell[2].innerHTML
		var away = table.row[rowNum].cell[3].innerHTML

		if (home > away){
			w = w + 1
			table.row[rowNum].cell[4] += "CU Boulder"
		}
		else{
			table.row[rowNum].cell[4] += table.row[rowNum].cell[1]
			l = l + 1
		}
	}
	var changewin = document.getElementById("wins")
	changewin.innerHTML += w
	var changel = document.getElementById("losses")
	changel.innerHTML += l
...
function loadPlayersPage(){
	var playermen = document.getElementById("player_section");
	var insert = "";

	for(var i = 0; i < players.length; i++){
		var play = players[i].name;
		insert = insert + <a class = "'dropdown-item' href ='#' onclick = 'switchPlayers("+ i +")'>;
	}
	playermen.innerHTML=insert;
}
...
function switchPlayers(playerNum){
	...
	var re = Math.floor(players[playerNum].receiving_yards/players[playerNum].games_played);

	document.getElementById("avg_p_yards").innerHTML = p;
	document.getElementById("avg_r_yards").innerHTML = r;
	document.getElementById("avg_rec_yards").innerHTML = re;
